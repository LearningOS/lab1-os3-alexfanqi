...
struct TaskManagerInner {
    ...
    current_task: usize,
    syscall_stats: [[u32; MAX_SYSCALL_NUM];MAX_APP_NUM],
}
        ...
        TaskManager {
            ...
            inner: unsafe {
                UPSafeCell::new(TaskManagerInner {
                    tasks,
                    current_task: 0,
                    syscall_stats: [[0; MAX_SYSCALL_NUM];MAX_APP_NUM],
                })
            },
        }
    };
...
/*pub fn update_time_stats() {
    ...
    let current = inner.current_task;
    let mut cur_task = inner.tasks[current];
    cur_task.task_total_time += current_time - inner.prev_time;
    inner.prev_time = current_time;
} */

pub fn update_syscall_stats(syscall_id: usize) {
    let mut inner = TASK_MANAGER.inner.exclusive_access();
    let current = inner.current_task;
    inner.syscall_stats[current][syscall_id] += 1;
}
...
pub fn get_current_task_info(ti: &mut TaskInfo) -> isize {
    ...
    ti.syscall_times = inner.syscall_stats[current];
